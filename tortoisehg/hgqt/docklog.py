...
class _LogWidgetForConsole(cmdui.LogWidget):
    ...
    def commandText(self):
        ...
        if l >= 0:
            return self.text(l)[len(self._prompt):].rstrip('\n')
        else:
            return ''
...
class ConsoleWidget(QWidget, qtlib.TaskWidget):
    ...
    @pyqtSlot(str, int)
    def historySearch(self, text: str, direction: int):
    ...
    @pyqtSlot(str)
    def completeCommandText(self, text: str):
    ...
    def _extproc(self):
        ...
        extproc.errorOccurred.connect(self._handleExtprocError)
    ...
    @pyqtSlot(str)
    def _runcommand(self, cmdline: str):
        self._commandIdx = 0
        try:
            args = hglib.parsecmdline(cmdline, self._workingDirectory())
        except ValueError as e:
            self.closePrompt()
            self._logwidget.appendLog(hglib.exception_str(e) + '\n', 'ui.error')
            self.openPrompt()
            return
        if not args:
            self.openPrompt()
...
class LogDockWidget(QDockWidget):
    ...
    @pyqtSlot(str)
    def _createConsoleFor(self, root: str):
        repoagent = self._repomanager.repoAgent(root)
        assert repoagent
        self._createConsole(repoagent)

    @pyqtSlot(str)
    def _destroyConsoleFor(self, root: str):
        w = self._findConsoleFor(root)
