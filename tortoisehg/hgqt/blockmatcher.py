...
class BlockList(QWidget):
    ...
    def setPageStep(self, pagestep: int) -> None:
    ...
    def paintEvent(self, event):
        ...
        for typ, alo, ahi in self._blocks:
            ...
            p.setBrush(color)
            p.drawRect(1, int(alo * sy), w - 1, int((ahi - alo) * sy))
...
class BlockMatch(BlockList):
    ...
    def paintEvent(self, event):
        ...
        h = self.height()
        p = QPainter(self)
        p.setRenderHint(qtlib.QtPainterRenderHint.Antialiasing)
    ...
    def setMaximum(self, maximum: int, side: str = 'left') -> None:
        self._maximum[side] = maximum
        self.update()
        self.rangeChanged.emit(self._minimum[side], self._maximum[side], side)

    def setMinimum(self, minimum: int, side: str = 'left') -> None:
    ...
    def setPageStep(self, pagestep: int, side: str = 'left') -> None:
