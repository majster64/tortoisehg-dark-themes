...
class BisectDialog(QDialog):
    ...
    def _lookupRevision(self, changeid):
        try:
            ctx = hglib.revsymbol(self.repo, hglib.fromunicode(changeid))
            return ctx.rev()
        except (error.LookupError, error.RepoLookupError) as e:
            self._stbar.showMessage(hglib.exception_str(e))
        except error.Abort as e:
            self._stbar.showMessage(hglib.exception_str(e, show_hint=True))

    @pyqtSlot()
    def _verifyGood(self):
        self.goodrev = self._lookupRevision(self._gle.text().strip())
    ...
    @pyqtSlot()
    def _verifyBad(self):
        self.badrev = self._lookupRevision(self._ble.text().strip())
    ...
    def _markRevision(self, button: QAbstractButton) -> None:
        self._nextbuttons.setEnabled(False)
