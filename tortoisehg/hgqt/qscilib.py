...
class Scintilla(ScintillaCompat):
    ...
    def _buildEditorOptionsMenu(self, menu):
        ...
        for name, mode in ((_('&None', 'wrap mode'), qsci.WrapMode.WrapNone),
                           (_('&Word'), qsci.WrapMode.WrapWord),
                           (_('&Character'), qsci.WrapMode.WrapCharacter)):
        ...
        for name, mode in ((_('&Visible'), qsci.WhitespaceVisibility.WsVisible),
                           (_('&Invisible'), qsci.WhitespaceVisibility.WsInvisible),
                           (_('&AfterIndent'), qsci.WhitespaceVisibility.WsVisibleAfterIndent)):
        ...
        if not self.isReadOnly():
            ...
            for name, mode in ((_('&Windows'), qsci.EolMode.EolWindows),
                               (_('&Unix'), qsci.EolMode.EolUnix),
                               (_('&Mac'), qsci.EolMode.EolMac)):
    ...
    def saveSettings(self, qs, prefix):
        if QT_VERSION >= 0x60000:
            value = lambda e: e.value  # pytype: disable=attribute-error
        else:
            value = lambda e: e
    ...
    def highlightText(self, match, icase=False):
        ...
        try:
            ...
            if icase:
                flags |= re.IGNORECASE
            pat = re.compile(match.encode('utf-8'), flags)
        except re.error:
        ...
        for m in pat.finditer(self.text().encode('utf-8')):
            self.SendScintilla(self.SCI_INDICATORFILLRANGE,
    ...
    def setDefaultEolMode(self):
        if self.lines():
            mode = qsciEolModeFromLine(self.text(0))
        else:
...
def readFile(editor, filename, encoding=None):
    ...
    try:
        ...
        if f.error() != QFileDevice.FileError.NoError:
            qtlib.WarningMsgBox(_('Unable to read file'),
    ...
    try:
        ...
        qtlib.WarningMsgBox(_('Unable to write file'),
                            _('Could not translate the file content to '
                              'native encoding.'),
                            hglib.exception_str(inst), parent=editor)
        return False
