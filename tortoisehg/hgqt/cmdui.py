...
class ThgStatusBar(QStatusBar):
    ...
    def setRepoBusy(self, root: str, busy):
        if busy:
    ...
    @pyqtSlot(str)
    def clearRepoProgress(self, root: str):
    ...
    def progress(self, topic: str, pos, item: str, unit: str, total, root=None):
    ...
    def setRepoProgress(self, root: str, progress):
        self.progress(*(progress + (root,)))
...
class InteractiveUiHandler(cmdcore.UiHandler):
    ...
    def setPrompt(self, text: str, mode, default: Optional[str]=None):
        self._prompttext = text
        self._promptmode = mode
        self._promptdefault = default if default else ''
    ...
    def _getChoiceInput(self):
        ...
        # button to close prompt dialog with empty response (i.e. abort)
        dlg.addButton(QMessageBox.StandardButton.Abort)
        dlg.setEscapeButton(QMessageBox.StandardButton.Abort)
        dlg.exec()
        button = dlg.clickedButton()
        if button and dlg.buttonRole(button) == QMessageBox.ButtonRole.ActionRole:
            return button.response

    def _parentWidget(self) -> Optional[QWidget]:
        # TODO: Drop this function if _uiparentref() can be typed as a QWidget
        p = self._uiparentref() if self._uiparentref else None
        while p and not p.isWidgetType():
            p = p.parent()

        return cast(Optional[QWidget], p)
...
class CmdControlDialog(QDialog):
    ...
    command.  exec() returns 0 on success.
    """
